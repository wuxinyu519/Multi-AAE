...
@tf.function
def train_step(model, batch_x,batch_y,lam, optimizers, std, NUM_OF_D, style, losses,util):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        z_distribution = tf.concat([z, tf.one_hot(batch_y, 10)], axis=1)
        print(z_distribution.shape)
        # real z label samples
        label_sample = [np.random.randint(0, 10, size=[batch_x.shape[0]]) for _ in range(NUM_OF_D)]
        label_sample_one_hot = [tf.one_hot(label_sample[i], 10) for i in range(NUM_OF_D)]
        real = [util.sample_mixture_of_normals(batch_x.shape[0], label_sample[i], 10) for i in range(NUM_OF_D)]
        real_distribution = [tf.concat(
            [real[i], label_sample_one_hot[i]], axis=1
        ...
        dc_real = [model.discriminator[ind](real_distribution[ind], training=True) for ind in
                   range(NUM_OF_D)]
        dc_fake = [model.discriminator[ind](z_distribution, training=True) for ind in
                   range(NUM_OF_D)]
        # Discriminator Loss
...
def train(args):
    ...
    UNIQUE_RUN_ID = f'MAAE_{args.dataset}_semi_{args.num_of_d}d_{args.style}_{args.latent_dim}z'
    PROJECT_ROOT = Path.cwd()
    output_dir = Path(args.output_dir)
    experiment_dir = output_dir / f'MAAE_{args.dataset}_semi_{args.num_of_d}d_{args.style}_{args.latent_dim}z'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_lr', type=float, default=0.0002, help='Learning rate for generator')
    parser.add_argument('--dc_lr', type=float, default=0.00004, help='Learning rate for discriminator')
